...
| --- | --- | --- | --- |
| 1 | Traffic-Flow-Logging (Management-injizierte Flow-Konfig) | gemergt auf `origin/main` (`b10e53168`, PR #1) | `fork/integrations/`, `go.mod` |
...
- Eigenes Replace-Modul `fork/integrations/` (Modulpfad
...
2. Nach dem Merge prüfen: `replace … => ./fork/integrations` noch in der Root-
  `go.mod`; `fork/integrations/` kompiliert gegen geänderte Upstream-API der
  Integrations-Schnittstelle (brechen laut statt still, Dateilayout ist
  gespiegelt).
